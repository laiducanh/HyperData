...
class ResultDialog(Dialog):
    def __init__(self, xdata, ydata, interpolator: Akima1DInterpolator,  parent=None):
        super().__init__(parent)
        if not interpolator:
            self.main_layout.addWidget(BodyLabel("Could not determine Akima interpolator."))
        else:            
            self.canvas = Canvas()
    ...
    def plot(self, xdata, ydata, interpolator: Akima1DInterpolator):
        # clear plot
